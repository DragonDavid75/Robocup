...
class BallTask(BaseTask):
    ...
    def update(self):
        ...
        # wait until drive is complete, and then close the gripper
            ...
            if not self.motion_controller.is_busy():
                print("[TASK] turn around (counter-clockwise)")
                self.motion_controller.turn_in_place(math.radians(170))
                self.state = 13
        
        elif self.state == 13:
            if not self.motion_controller.is_busy():
                print("[TASK] drive distance found in first image")
                self.motion_controller.drive_distance(self.return_distance, 0.2)
                self.state = 14

        elif self.state == 14:
            if not self.motion_controller.is_busy():
                print("[TASK] turn right 45")
                self.motion_controller.turn_in_place(math.radians(-45))
                self.state = 15

        elif self.state == 15:
            if not self.motion_controller.is_busy():
                print("[TASK] drive until line is found")
