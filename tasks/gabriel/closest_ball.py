...
class ClosestBall(BaseTask):
    def __init__(
        ...
        # Keeping track of the distance travelled after taking the first image
        self.return_distance = 0.0
        # self.target_color = ['red', 'blue']
        ...
        self.colors = {
            ...
            'blue': {'lower1': (90, 50, 127), 'upper1': (110, 255, 255)},
    ...
    def start(self):
        ...
        if ok:
            ...
            # Look for both red and blue balls
            shortest_distance = 0
            shortest_coords = []
            for i in range(len(self.target_color)):
                # 3. Detection (detects coordinates of blue and red balls)
                coords = self.detect_ball(roi_frame, self.target_color[i])
                ...
                if coords:
                    ...
                    # Find shortest distance to a ball
                    # Only want shortest_x and y if some coords were detected
                    if i == 0: 
                        shortest_distance = math.hypot(world_x/100, world_y/100)
                        shortest_coords = [world_x, world_y, u, v_world_space, self.target_color[i]]
                    if i == 1:
                        if shortest_distance == 0:
                            shortest_distance = math.hypot(world_x/100, world_y/100)
                            shortest_coords = [world_x, world_y, u, v_world_space, self.target_color[i]]
                        elif shortest_distance > math.hypot(world_x/100, world_y/100):
                            shortest_distance = math.hypot(world_x/100, world_y/100)
                            shortest_coords = [world_x, world_y, u, v_world_space, self.target_color[i]]
            if not shortest_coords:
                Balls = False
            else: 
                Balls = True
            if Balls:
                print(f"[TASK] Detected {shortest_coords[4]} ball at pixel ({shortest_coords[2]:.1f}, {shortest_coords[3]:.1f}) -> world ({shortest_coords[0]:.2f}, {shortest_coords[1]:.2f})")

        if self.detect_ball_cam:
            # Set world coordinates of target balls as the closest ones
            world_x = shortest_coords[0]
            world_y = shortest_coords[1]

            print(f"[TASK] Closest {shortest_coords[4]} ball identified")

            self.closest_ball = shortest_coords[4]
            self.world.first_ball = shortest_coords[4]
            ...
            self.total_distance = self.drive_distance_m
    ...
    def update(self):
        ...
        # drive to the ball, not using the line
        elif self.state == 5:
            if self.drive_distance_m > 1e-6:
                ...
                # Keeping track of distance travelled after the first image
                self.return_distance = self.drive_distance_m
            self.state = 6
        ...
        # execute sequence to drive back and go to a positino where the blue ball aruc code is visible from
        elif self.state == 11:
            if not self.motion_controller.is_busy():
                print("[TASK] Stage 2 complete, attempting to grip the ball")
                self.servo_controller.servo_control(2, -1000, 500)
                time.sleep(0.5)
                self.motion_controller.drive_distance(self.total_distance, -0.3) # back up a bit after gripping
                self.state = 12

        elif self.state == 12:
            if not self.motion_controller.is_busy():
                self.motion_controller.turn_in_place(math.radians(-self.total_turn))
                print("[TASK] DriveToBall completed")
                self.state = 13
            ...
            if not self.motion_controller.is_busy():
                if self.closest_ball is not None:
                    if self.closest_ball == 'blue':
                        print(" [TASK] task done until closest blue ball found")
                        self.motion_controller.turn_in_place(math.radians(-50))
                        self.state = 14
                    elif self.closest_ball == 'red':
                        print(" [TASK] task done until closest red ball found")
                        self.state = 14
